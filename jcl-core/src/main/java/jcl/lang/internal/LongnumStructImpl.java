...
public final class LongnumStructImpl extends BuiltInClassStruct implements BignumStruct {
	...
	public IntegerStruct gcd(final IntegerStruct integer) {
		if (integer instanceof FixnumStructImpl) {
			final long gcd = ArithmeticUtils.gcd(value, integer.intValue());
	...
	public IntegerStruct lcm(final IntegerStruct integer) {
		if (integer instanceof FixnumStructImpl) {
			final long lcm = ArithmeticUtils.lcm(value, integer.intValue());
	...
	public IntegerStruct isqrt() {
		final long isqrt = (long) Math.floor(Math.sqrt(value));
	...
	public FloatStruct floatingPoint(final FloatStruct prototype) {
		// TODO
		return FloatStructImpl.valueOf(new Apfloat((double) value), prototype);
	}

	@Override
	public RealStruct mod(final RealStruct divisor) {
		if (divisor instanceof FixnumStructImpl) {
			final long mod = value % ((FixnumStructImpl) divisor).intValue();
