...
@Component
public final class CharEqualFunction extends FunctionStruct {
	...
	public SymbolStruct<?> getFunctionSymbol() {
		...
		final SymbolStruct<?> symbol = aPackage.intern("CHAR-EQUAL").getSymbol();
	...
	public LispStruct apply(final LispStruct... lispStructs) {
		...
		return CharacterStruct.isEqualToIgnoreCase(characters) ? TStruct.INSTANCE : NILStruct.INSTANCE;
	}
	...
	private CharacterStruct[] getCharacters(final LispStruct... lispStructs) {
		...
		for (int i = 0; i < lispStructs.length; i++) {
			...
			validator.validateTypes(lispStruct, "CHAR-EQUAL", "Character", CharacterType.INSTANCE);
