...
@Component
public final class CharGTEqFunction extends FunctionStruct {
	...
	public SymbolStruct<?> getFunctionSymbol() {
		...
		final SymbolStruct<?> symbol = aPackage.intern("CHAR>=").getSymbol();
	...
	public LispStruct apply(final LispStruct... lispStructs) {
		...
		return CharacterStruct.isGreaterThanOrEqualTo(characters) ? TStruct.INSTANCE : NILStruct.INSTANCE;
	}
	...
	private CharacterStruct[] getCharacters(final LispStruct... lispStructs) {
		...
		for (int i = 0; i < lispStructs.length; i++) {
			...
			validator.validateTypes(lispStruct, "CHAR>=", "Character", CharacterType.INSTANCE);
