...
@Component
public final class CharNotEqualFunction extends FunctionStruct {
	...
	public SymbolStruct<?> getFunctionSymbol() {
		...
		final SymbolStruct<?> symbol = aPackage.intern("CHAR-NOT-EQUAL").getSymbol();
	...
	public LispStruct apply(final LispStruct... lispStructs) {
		...
		return CharacterStruct.isNotEqualToIgnoreCase(characters) ? TStruct.INSTANCE : NILStruct.INSTANCE;
	}
	...
	private CharacterStruct[] getCharacters(final LispStruct... lispStructs) {
		...
		for (int i = 0; i < lispStructs.length; i++) {
			...
			validator.validateTypes(lispStruct, "CHAR-NOT-EQUAL", "Character", CharacterType.INSTANCE);
