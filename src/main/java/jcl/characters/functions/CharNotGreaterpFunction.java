...
@Component
public final class CharNotGreaterpFunction extends FunctionStruct {
	...
	public SymbolStruct<?> getFunctionSymbol() {
		...
		final SymbolStruct<?> symbol = aPackage.intern("CHAR-NOT-GREATERP").getSymbol();
	...
	public LispStruct apply(final LispStruct... lispStructs) {
		...
		return CharacterStruct.isLessThanOrEqualToIgnoreCase(characters) ? TStruct.INSTANCE : NILStruct.INSTANCE;
	}
	...
	private CharacterStruct[] getCharacters(final LispStruct... lispStructs) {
		...
		for (int i = 0; i < lispStructs.length; i++) {
			...
			validator.validateTypes(lispStruct, "CHAR-NOT-GREATERP", "Character", CharacterType.INSTANCE);
