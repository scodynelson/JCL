...
@Component
public final class CharNotLesspFunction extends FunctionStruct {
	...
	public SymbolStruct<?> getFunctionSymbol() {
		...
		final SymbolStruct<?> symbol = aPackage.intern("CHAR-NOT-LESSP").getSymbol();
	...
	public LispStruct apply(final LispStruct... lispStructs) {
		...
		return CharacterStruct.isGreaterThanOrEqualToIgnoreCase(characters) ? TStruct.INSTANCE : NILStruct.INSTANCE;
	}
	...
	private CharacterStruct[] getCharacters(final LispStruct... lispStructs) {
		...
		for (int i = 0; i < lispStructs.length; i++) {
			...
			validator.validateTypes(lispStruct, "CHAR-NOT-LESSP", "Character", CharacterType.INSTANCE);
