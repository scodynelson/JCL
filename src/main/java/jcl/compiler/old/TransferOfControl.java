...
public class TransferOfControl {
	 ...
	 */
	private static Stack<TOCRecord> TOCMgmt = new Stack<>();
	 ...
	 * This is the exception that will be rethrown at the end of the cleanup form (if one is going to be rethrown at
	 * all).
	 */
	 ...
	 * the callers will pass the type and this method will find and remove the first instance of that corresponding type
	 * on the stack.
	 */
	...
	public static Object isMine(final Throwable ex) {
		...
		final TOCRecord current = TOCMgmt.peek();          //The first record on the stack
		...
		if (matches(current, ex)) {
			...
			if (ex instanceof TRFException) {
				final Object tempValue = ((TRFException) ex).getValue();           //Return value of exception
				if (tempValue != null) {
					return tempValue;
				} else {
					return new Object();
			...
			final Class<?> tempThrow = ex.getClass();
			if (tempThrow != null) {
	...
	private static boolean matches(final TOCRecord current, final Throwable ex) {
		return ((ex instanceof ThrowException) && (current.type.compareTo(CATCH) == 0) && Objects.equals(current.value, ((ThrowException) ex).getTag()))
				|| ((ex instanceof ReturnFromException) && (current.type.compareTo(BLOCK) == 0) && Objects.equals(current.value, ((ReturnFromException) ex).getTag()))
				|| ((ex instanceof GoException) && (current.type.compareTo(TAGBODY) == 0) && current.value.toString().contains(((GoException) ex).getTag().toString()));
	}

	/**
	 * Immediately after the finally block from the unwind-protect form begins execution, this method is called.
	 * This method will traverse up the TOCMgmt stack and disable every single TOCRecord until it finds the first
	 * instance of a
	 * TOCRecord that matches the current returnException value. All other methods (such as isMine) will always check
	 * the valid
	 * bit before validating an entry in the TOCMgmt stack. This will ensure that the cleanup form from unwind-protect
	 * is unable
	...
	public static void disableExitPoints(final Throwable ex) {
		...
		while (!found && !TOCMgmt.empty()) {
			TOCRecord tempRecord = TOCMgmt.pop();
	...
	/**
	 * If an exception was caught (and was not handled by the current TOC), it would have been stored as the
	 * returnException.
	 * If this was the case, once the finally code of the TOC is completed, this will be the last call made before going
	 * on to
	...
	/**
	 * This is simply a struct that holds the information about an instance of a control transfer that will be held on
	 * the TOCMgmt stack.
	 */
