...
public class LambdaListParser {
	...
	private static RequiredParseResult parseRequiredBindings(final AnalysisBuilder analysisBuilder, final Iterator<LispStruct> iterator, final int position) {
		...
		while (iterator.hasNext() && !isLambdaListKeyword(currentElement)) {
			...
			environmentStack.peek().addBinding(currentParam, newBindingsPosition, NILStruct.INSTANCE);
		}
	...
	private static OptionalParseResult parseOptionalBindings(final SemanticAnalyzer semanticAnalyzer, final AnalysisBuilder analysisBuilder, final Iterator<LispStruct> iterator, final int position) {
		...
		while (iterator.hasNext() && !isLambdaListKeyword(currentElement)) {
			if (currentElement instanceof SymbolStruct) {
				...
				environmentStack.peek().addBinding(varNameCurrent, newBindingsPosition, parameterValueInitForm);
				...
				if (!thirdInCurrent.equals(NullStruct.INSTANCE)) {
					...
					environmentStack.peek().addBinding(suppliedPCurrent, newBindingsPosition, NILStruct.INSTANCE);
				}
