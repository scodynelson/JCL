...
@Component
public class LabelsAnalyzer implements SpecialOperatorAnalyzer {
	...
	@Override
	public LabelsElement analyze(final SemanticAnalyzer analyzer, final ListStruct input, final AnalysisBuilder analysisBuilder) {
		...
		try {
			...
			return new LabelsElement(labelsVars, analyzedBodyForms, labelsEnvironment);
		} finally {
	...
	private LabelsElement.LabelsVar getLabelsVar(final LispStruct functionParameter,
		...
		final int newBindingsPosition = EnvironmentAccessor.getNextAvailableParameterNumber(labelsEnvironment);
		...
		return new LabelsElement.LabelsVar(functionNameSE, functionInitForm);
	}
