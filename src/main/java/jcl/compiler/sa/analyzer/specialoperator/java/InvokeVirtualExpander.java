...
public class InvokeVirtualExpander extends MacroFunctionExpander<InvokeInstanceMethodCallStruct> {

	public static final InvokeVirtualExpander INSTANCE = new InvokeVirtualExpander();

	@Override
	public SymbolStruct getFunctionSymbol() {
		return CommonLispSymbols.JINVOKE_VIRTUAL;
	}

	@Override
	public InvokeInstanceMethodCallStruct expand(final ListStruct form, final Environment environment) {
		final Iterator<LispStruct> iterator = form.iterator();
		iterator.next(); // JINVOKE-VIRTUAL SYMBOL

		// TODO: should do a FormAnalyze here??
		final LispStruct method = iterator.next();
		if (!(method instanceof ConsStruct)) {
			throw new TypeErrorException("INVOKE-VIRTUAL: Invalid Java Method. Got: " + method);
		}
